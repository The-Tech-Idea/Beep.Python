#!/usr/bin/env python3
"""
Beep.Python Runtime Named Pipe Server
...
def run_windows_pipe_server(pipe_name: str):
    """Run named pipe server on Windows"""
    try:
        import win32pipe
        import win32file
        import pywintypes
    except ImportError:
        print("ERROR: pywin32 is required on Windows. Install with: pip install pywin32", file=sys.stderr, flush=True)
        sys.exit(1)
    ...
    while True:
        ...
        try:
            ...
            print(f'Pipe error: {e}', file=sys.stderr, flush=True)
            traceback.print_exc()
        finally:
            try:
...
def run_unix_socket_server(pipe_name: str):
    """Run Unix domain socket server on Linux/macOS"""
    import socket
    import os
...
def main():
    parser = argparse.ArgumentParser(description="Beep.Python Runtime Named Pipe Server")
    parser.add_argument('--pipe-name', default='beep-python-pipe', help='Named pipe name')
