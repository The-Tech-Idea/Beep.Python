...
if 'data' in globals() and target_column in data.columns:
    ...
    X_resampled, y_resampled = ros.fit_resample(data.drop(columns=[target_column]), data[target_column])
...
if 'train_data' in globals() and target_column in train_data.columns:
    ros = RandomOverSampler(sampling_strategy=sampling_strategy, random_state=42)
    X_resampled, y_resampled = ros.fit_resample(train_data.drop(columns=[target_column]), train_data[target_column])
